# train_cogvideox_sft_video_to_video.py
...
# Import the custom video-to-video pipeline and a helper for loading videos
...
def run_validation(
    ...
    for validation_video, validation_prompt in zip(validation_videos, validation_prompts):
        ...
        validation_video = load_video(validation_video,
                height=args.height,
                width=args.width,
...
class CollateFunction:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        # 'image' in original SFT script is replaced by 'input_video'
        input_videos = [x["image"] for x in data[0]]
        input_videos = torch.stack(input_videos).to(dtype=self.weight_dtype, non_blocking=True)

        # 'video' in original SFT script is replaced by 'output_video'
        output_videos = [x["video"] for x in data[0]]
...
def main(args):
    ...
    # text_encoder and vae frozen, we train the transformer fully (SFT)
    ...
    # For mixed precision training we cast the non-trainable parts to half-precision
    ...
    # If fp16: upcast trainable parameters
    if args.mixed_precision == "fp16":
    ...
    # If we don't need VAE + text_encoder in memory, remove them
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                # Renamed "images" -> "input_videos" and "videos" -> "output_videos"
                ...
                # Encode videos with VAE if not loaded as latents
                if not args.load_tensors:
                    # input / source video latents
                ...
                # same logic as image->video, but here it's video->video:
                # put the source video latents in the first frame, then zero the rest
                ...
                # Encode prompts with the text encoder if needed
                if not args.load_tensors:
                ...
                # Sample noise for forward diffusion
            ...
            last_lr = lr_scheduler.get_last_lr()[0] if lr_scheduler else args.learning_rate
            logs.update(
                {
    ...
    if accelerator.is_main_process:
        ...
        # Run inference
        validation_outputs = []
        if args.validation_prompt and args.num_validation_videos > 0 and hasattr(args, "validation_videos"):
            validation_prompts = args.validation_prompt.split(args.validation_prompt_separator)
            validation_videos = args.validation_videos.split(args.validation_prompt_separator)
