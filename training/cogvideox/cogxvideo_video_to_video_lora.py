...
def save_model_card(
    ...
    """
    Saves a model card to the repo folder, displaying final validation videos, licensing, etc.
    """
    widget_dict = []
    ...
    model_card = load_or_create_model_card(
        ...
        from_training=True, license="other",
        base_model=base_model,
        prompt=validation_prompt,
        model_description=model_description,
        widget=widget_dict)

    tags = [ "text-to-video", "image-to-video", "diffusers-training", "diffusers", "lora", "cogvideox", "cogvideox-diffusers", "template:sd-lora"]
...
def log_validation(
        ...
        pipe: CogVideoXVideoToVideoPipeline,
        args: Dict[str, Any],
        pipeline_args: Dict[str, Any],
        is_final_validation: bool = False,
        ):
    """ Runs validation by generating a few videos from pipe. """
    ...
    for _ in range(args.num_validation_videos):
        vid = pipe(**pipeline_args, generator=generator, output_type="np").frames[0]
        videos.append(vid)

    # Logging to W&B if necessary
...
def run_validation(
    ...
    """ Creates a CogVideoXVideoToVideoPipeline from the current checkpoint, runs validation, and logs outputs. """
    accelerator.print("===== Memory before validation =====")
    print_memory(accelerator.device)
    torch.cuda.synchronize(accelerator.device)

    # Create pipeline for video-to-video:
    ...
    # Slicing/tiling/model CPU offload
    ...
    # Loop over all validation prompts & videos
    ...
    for validation_video, validation_prompt in zip(validation_videos, validation_prompts):
        ...
        input_validation_video = load_video(validation_video, args.height, args.width, args.max_num_frames, args.skip_frames_start, args.skip_frames_end)
        
        pipeline_args = {
            "video": input_validation_video,
            "prompt": validation_prompt,
...
class CollateFunction:
    """ Minimal collate for input videos & output videos, plus the prompt embeddings or strings. """ 
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        # "image" in the dataset is now the "input_video"
        input_videos = [x["input_video"] for x in data[0]]
        input_videos = torch.stack(input_videos).to(dtype=self.weight_dtype, non_blocking=True)

        # "video" in the dataset is the "output_video"
        output_videos = [x["output_video"] for x in data[0]]
        output_videos = torch.stack(output_videos).to(dtype=self.weight_dtype, non_blocking=True)
...
def main(args):
    ...
    # Security measure for wandb + huggingface hub credentials
    if args.report_to == "wandb" and args.hub_token is not None:
        raise ValueError( "You cannot use both --report_to=wandb and --hub_token due to a security risk of exposing your token."
    ...
    # CogVideoX-5b is typically bfloat16, CogVideoX-2b is float16
    ...
    # Optionally remove learned positional embeddings if ignoring them
    ...
    # Choose weight dtype for inference modules
    weight_dtype = torch.float32
    if accelerator.state.deepspeed_plugin:
        # Deepspeed plugin decides
        if (
    ...
    # Custom save & load hooks for LoRA
    ...
    if args.use_cpu_offload_optimizer:
        lr_scheduler = None
        accelerator.print(
            "CPU Offload Optimizer cannot be used with DeepSpeed or builtin PyTorch LR Schedulers. "
            "If you are training with those settings, they will be ignored."
        )
    else:
    ...
    # Re-check total steps after dataloader changes
    num_update_steps_per_epoch = math.ceil(len(train_dataloader) / args.gradient_accumulation_steps)
    ...
    # If we don't need VAE + text_encoder in memory
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                # Convert images & videos to latents if not loaded from disk as latents
                if not args.load_tensors:
                    # input video latents
                    input_videos = input_videos.permute(0, 2, 1, 3, 4)  # [B, C, F, H, W]
                    input_video_noise_sigma = torch.normal(
                        mean=-3.0, std=0.5, size=(input_videos.size(0),), device=accelerator.device, dtype=weight_dtype
                    )
                    input_video_noise_sigma = torch.exp(input_video_noise_sigma)
                    noisy_input_videos = input_videos + torch.randn_like(input_videos) * input_video_noise_sigma[:, None, None, None, None]
                    input_videos_latent_dist = vae.encode(noisy_input_videos).latent_dist

                    # output video latents
                    output_videos = output_videos.permute(0, 2, 1, 3, 4)  # [B, C, F, H, W]
                    output_videos_latent_dist = vae.encode(output_videos).latent_dist
                ...
                input_video_latents = input_video_latents.permute(0, 2, 1, 3, 4).to(memory_format=torch.contiguous_format, dtype=weight_dtype)

                output_video_latents = output_videos_latent_dist.sample() * VAE_SCALING_FACTOR
                output_video_latents = output_video_latents.permute(0, 2, 1, 3, 4).to(memory_format=torch.contiguous_format, dtype=weight_dtype)

                # input_video_latents = torch.cat([input_video_latents, output_video_latents], dim=1)

                # possibly dropout image latents
                ...
                # offset token embedding if needed
                ...
                if not args.use_cpu_offload_optimizer and lr_scheduler is not None:
                    lr_scheduler.step()
    ...
    if accelerator.is_main_process:
        ...
        if args.validation_prompt and args.num_validation_videos > 0:
            ...
            for validation_video, validation_prompt in zip(validation_videos, validation_prompts):
                ...
                input_validation_video = load_video(validation_video,
                        height=args.height,
                        width=args.width,
                        max_num_frames=args.max_num_frames,
